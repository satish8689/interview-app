...
yourArray = [
  ...
  },
  {
    ...
    "type": "javascript",
    "answer": "<div ><table><thead><tr><th style='text-align: center;'>var</th><th>let</th><th>const</th></tr></thead><tbody><tr><td style='text-align: center;'>var has the function or global scope.</td><td>let's have the block scope.</td><td>const variable has the block scope.</td></tr><tr><td style='text-align: center;'>It gets hoisted to the top of its scope and initialized undefined.</td><td>It also got hoisted to the top of its scope but didn't initialize.</td><td>It also got hoisted to the top of its scope but didn't initialize.</td></tr><tr><td style='text-align: center;'>It can be updated or re-declared.</td><td>It can only be updated and can't be re-declared.</td><td>It can't be updated or re-declared.</td></tr><tr><td style='text-align: center;'>It's an old way to declare a variable.</td><td>It's a new way to declare variables introduced in ES6.</td><td>It's also a new way to declare a variable, which introduces in ES6.</td></tr><tr><td style='text-align: center;'>Can be declared without initialization.</td><td>Can be declared without initialization.</td><td>Cannot be declared without initialization.</td></tr><tr><td style='text-align: center;'>Can be accessed without initialization as its default value is “undefined”.</td><td>Cannot be accessed without initialization, leading to ‘referenceError’.</td><td>Cannot be accessed without initialization, as it cannot be declared without initialization in the first place.</td></tr></tbody></table> <br> https://www.scaler.com/topics/javascript/</div>"
  },
  {
    "question": "What is NaN property in javascript?",
    "type": "javascript",
    "answer": "<div >NaN property represents the “Not-a-Number” value. To check if a value is NaN, we use the isNaN() function, Note- isNaN() function converts the given value to a Number type, and then equates to NaN.</div>"
  },
  {
  ...
  },
  {
    ...
    "type": "node",
    "answer": "<div className='node-answer'> Some of the reasons why Node.js is preferred include:                        <ul>                            <li>Node.js is very fast</li>                            <li>Node Package Manager has over 50,000 bundles available at the developer’s disposal</li>                            <li>Perfect for data-intensive, real-time web applications, as Node.js never waits for an API to return data</li>                            <li>Better synchronization of code between server and client due to same code base</li>                            <li>Easy for web developers to start using Node.js in their projects as it is a JavaScript library</li>                        </ul>                    </div>"
  },
  {
  ...
  },
  {
    ...
    "type": "node",
    "answer": " <div className='node-answer'> The “require” command is used for importing external libraries. For example - “var http=require (“HTTP”).”  This will load the HTTP library and the single exported object through the HTTP variable.                    </div>"
  },
  {
  ...
  },
  {
    ...
    "type": "node",
    "answer": "<div >An event listener is a procedure in JavaScript that waits for an event to occur. A simple example of an event is a user clicking the mouse or pressing a key on the keyboard. </br> </br>  The <b>addEventListener()</b> is an inbuilt function in JavaScript which takes the event to listen for, and a second argument to be called whenever the described event gets fired. Any number of event handlers can be added to a single element without overwriting existing event handlers. </br> </br> <b>Syntax: </b> element.addEventListener(event, listener, useCapture); </br></br> <b>Parameters :</b> <ul> <li> event: event can be any valid JavaScript event. Events are used without “on” prefixes like using “click” instead of “onclick” or “mousedown” instead of “onmousedown”. </li> <li>listener(handler function) : It can be a JavaScript function that respond to the event occurring. </li> <li> useCapture: It is an optional parameter used to control event propagation. A boolean value is passed where “true” denotes capturing phase and “false” denotes the bubbling phase. </li> </ul> </div>"
  },
  {
    "question": "What is an Event Loop in Node.js?",
    "type": "node",
    "answer": " <div className='node-answer'>The event loop is the secret behind of JavaScript’s asynchronous programming. javascript executes all operations on a single thread, but using a few smart data structures, it gives us the illusion of multi-threading. </br> If we are discuss how it works in node js. So node js us the concept of Libuv, Libuv is a espesial library build for node js but we can use application as well it's provide a concept of non blooking i/o now it's build in c++ language </br> </br> If we are discuss about how  event loop works. so basicaly event loop monitaring callstack and callback queue </br> When i execute my code and any function firstly code store on call stack and all synchronous operations run one by one and asynchronous operations move on the web api with desired time when time completed after that promises and settimeout  move on the callback queue, If the Call Stack is empty, the Event Loop will take the task first from  micro task queue then from the macro queue  of callback queue. </br></br>  macroTasks: setTimeout, setInterval, setImmediate, requestAnimationFrame, I/O, UI rendering </br> microTasks: process.nextTick, Promises, queueMicrotask, MutationObserver </div>"
  },
  {
  ...
  },
  {
    ...
    "type": "node",
    "answer": "<div class=''> <p>Usually, Node.js allows single-threaded, non-blocking performance but running a single thread in a CPU cannot handle, Node.js comes with a<strong>&nbsp;child process</strong> module, which helps to run other processes in the current environment i.e. it can be used to communicate with other processes.&nbsp;</p><p>Let’s understand this with an example.&nbsp;</p><p>App1: &nbsp;</p><ul><li>Create a new file exec.js and import child_process module.&nbsp;</li><li>const cp = require(“child_process”);&nbsp;</li><li>The child process has a method exec to open another process.&nbsp;</li><li>cp.exec(“open <a href='http://www.google.com/' rel='nofollow'  arget='_blank'>http://www.google.com</a>”);&nbsp;</li><li>run node exec&nbsp;</li><li>And observe that google page is opened.&nbsp;</li><li>Imagine if we invoke a process that returns data then we can grab that data as well.&nbsp;</li></ul><pre class='nitro-offscreen'>cp.exec(“ls”, (err, data) =&gt; {&nbsp;  &nbsp;if (err) { throw err; }&nbsp;    Console.log(data);&nbsp;    }); </pre> Four type of child process - spawn(), fork(), exec(),  execFile() </div>"
  },
  {
  ...
  },
  {
    ...
    "type": "node",
    "answer": "<div > <ul> <li>Import the HTTP module</li> <li>Use createServer function with a callback function using request and response as parameters.</li> <li>Type “hello world.“ </li> <li>Set the server to listen to port 8080 and assign an IP address</li> </ul> </div>"
  },
  {
    "question": "How do you create a simple server in Node.js that returns Hello World?",
    "type": "node",
  ...
  },
  {
    ...
    "type": "node",
    "answer": "<div className='node-answer'> The process.nextTick() method is used to add a new callback function at the start of the next event queue. it is called before the event is processed. The setImmediate is called at the check phase of the next event queue. It is created in the poll phase and is invoked during the check phase. </div>"
  },
  {
  ...
  {
    "question": "What are React Hooks? Give an example of a Hook you have used ‘and explain its purpose.",
    "type": "react",
    "answer": ""
  },
  {
  ...
  },
  {
    "question": "SQL query to find second highest salary?",
    "type": "sql",
    "answer": "<div className='sql-box'></div>"
  }
]


// 🔹 Function to add unique ID
...
const updatedArray = addUniqueId(yourArray);

// 🔹 Output file path
const outputPath = path.join(__dirname, './interviewData.json');
